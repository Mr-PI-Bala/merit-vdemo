...
for (const [name, url] of services) {
  try {
    ...
    checks.push({ name, url, status: response.status, ok: response.ok, service: json?.service || null, version: json?.version || null, details: name === 'gateway' ? { backing_services: json?.backing_services || null } : name === 'store' ? { offerings: json?.store?.offerings ?? null, registrations: json?.store?.registrations ?? null } : null, error: json?.error || (response.ok ? null : text.slice(0, 120)) });
  } catch (error) {
